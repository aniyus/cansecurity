...
var _getUserFromSession = function(request){
   var session = request.session["X-CS-Auth"] ;
   if ( session ) {
			return session.user;
   } else{
            return session;
...
var _customAuthErrorReply = function(req,res,status,msg){
    if(res && res!=null){
      res.send(status, msg);
      return false;
    }
    else{
        req.errors={status:status,messge:msg};
        return true;  //ignore
    }
...
checkSelf = function (req, res, next) {
 // console.log("hereiam");
  var isSelf = false;
  var session = _getUserFromSession(req);
  if (session&& session!=null && session[fields.id] === req.param(params.id)) { 
    isSelf = true;
  }
  if(isSelf)
     next();
...
checkUserRoles = function (req, res, next, roles) {
    ...
    isRole = false,
    i, targetRoles = {};
    if (session && session!=null && (userRoles=session[fields.roles])){
  ...
  //  console.log("aniyus","role",isRole);
  if(isRole){
      next();
  }
  return (isRole);
...
checkParam = function (req, res, next, param) {
  var session = _getUserFromSession(req);
    ...
    id =null,
    i;
  if (!session ||  session==null || !(id=session[fields.id] )) { 
    return false;
  }
...
checkField = function (req, res, next, field, getObject) {
 var session = _getUserFromSession();
 if (!session ||  session==null ) { 
    return false;
  }
  var id = session[fields.id],
...
fns = {
  direct: {
    restrictToLoggedUiIn: function (req, res, next) {
      // If our user is authenticated
      // then everything is fine :)
      if (checkLoggedIn(req, res, next)) {
        next();
      }else{
           authCallback(req,res,401,errors.unauthenticated(),"not-authenticated",true);
      }
    },
    ...
    restrictToLoggedIn: function (req, res, next) {
      ...
      if (checkLoggedIn(req, res, next)) {
        next();
           ...
           authCallback(req,res,401,errors.unauthenticated(),"not-authenticated");
      }
    },
    ...
    restrictToSelf: function (req, res, next) {        
      ...
      if (checkLoggedIn(req, res, next)) {
        if (!checkSelf(req, res, next)) {
            authCallback(req,res,403,errors.unauthorized(),"not-me");
      ...
      return function (req, res, next) {
        if (checkLoggedIn(req, res, next)) {
          if (!checkUserRoles(req, res, next, roles)) {
              authCallback(req,res,403,errors.unauthorized(),"not-have-roles");
           // res.send(403, errors.unauthorized());
          }
        }else{
           authCallback(req,res,401,errors.unauthenticated(),"not-authenticated");
      ...
      return function (req, res, next) {
        if (checkLoggedIn(req, res, next)) {
          if (!checkParam(req, res, next, param)) {
              authCallback(req,res,403,errors.unauthorized(),"field-missing");
          //  res.send(403, errors.unauthorized());
          }
        }else{
           authCallback(req,res,401,errors.unauthenticated(),"not-authenticated");
    ...
    restrictToParamOrRoles: function (param, roles) {
      ...
      return function (req, res, next) {
        ...
        if (checkLoggedIn(req, res, next)) {
          if (!checkUserRoles(req, res, next, roles)) {
            if (!checkParam(req, res, next, param)) {
                authCallback(req,res,403,errors.unauthorized(),"restrictToParamOrRoles");
             // res.send(403, errors.unauthorized());
            }
          }
        }else{
      ...
      return function (req, res, next) {
        ...
        if (checkLoggedIn(req, res, next)) {
          if (!checkField(req, res, next, field, getField)) {
              authCallback(req,res,403,errors.unauthorized(),"restrictToField");
            //res.send(403, errors.unauthorized());
          }
        }else{
           authCallback(req,res,401,errors.unauthenticated(),"not-authenticated");
    ...
    restrictToFieldOrRoles: function (field, roles, getField) {
      ...
      return function (req, res, next) {
        ...
        if (checkLoggedIn(req, res, next)) {
          if (!checkUserRoles(req, res, next, roles)) {
            if (!checkField(req, res, next, field, getField)) {
                authCallback(req,res,403,errors.unauthorized(),"field-not-have-roles");
              // res.send(403, errors.unauthorized());
            }
          }
        }else{
...
module.exports = {
  init: function (config) {
    config = config || {};
    authCallback=config.authCallback||_customAuthErrorReply;
